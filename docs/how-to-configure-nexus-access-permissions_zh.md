# IDEA插件Gradle With Me / Gradle With Me Pro如何配置Nexus访问权限？

[返回主页](../README.md) / [返回Pro主页](../pro/README.md)

这里介绍的是当Nexus仓库开启了权限访问，若Gradle With Me(GPVP) / Gradle With Me Pro(GPVP)不进行权限配置，将无法正常获取数据的场景。若您的Nexus仓库未配置权限，则保持插件默认配置即可。
 ...
 **操作步骤：**
  ...
  1. 找到用户主目录/gpvp/search-nexus-cache.json中要配置用户权限的URL  
...
以上介绍了Gradle With Me(GPVP) / Gradle With Me Pro(GPVP)如何配置Nexus访问权限的几种方式，大家可根据实际情况选择~
