...
<p>
    <ul>
        ...
        <li>如果任何间接、特殊、偶然或不可抗拒或后果性的损失或对于任何特性的疏忽所造成的损坏，包括但不限于数据丢失、系统故障、服务中断、经济损失、商誉损害等，我们不承担任何法律责任及赔偿，需要您自行负责。
        </li>
        <li>您在本地设备上生成的数据（如存档、配置）仅存储于您的设备中。若设备丢失、损坏或软件被卸载，相关数据可能无法恢复。建议您定期自行备份重要数据。对于因未备份导致的数据丢失，我们不承担赔偿责任。</li>
        <li>使用软件产生的流量、宽带、话费等运营商费用，全部由用户自行承担。在法律允许范围内，我方所有赔偿总额不超过该笔授权对应的已付费用；非我方故意或重大过失造成的各类间接损失，不予赔付。</li>
    </ul>
</p>
...
<p>
    <ul>
        <li>本软件无需创建账户或登录即可使用。为优化使用体验、排查程序故障、统计使用情况，我们仅基于最小必要原则在必要范围内采集数据。若您希望停止数据收集，可通过关闭设备的网络权限或使用系统级的隐私设置来实现，但这可能导致部分需要联网的功能无法使用。您有权随时卸载本软件以终止所有数据交互。</li>
        <li>授权激活必填信息（仅用于授权校验、发放授权 KEY），其中：
        <br>
              软件序列号：软件序列号的生成依赖系统设备信息作为激活授权标识（仅用于识别机器身份，不提取各类敏感个人信息）。
        <br>
              电子邮箱地址：仅用于接收激活密钥、授权更新通知与售后问题沟通，不会发送营销广告。
        <br>
              订单交易号（如有）：核验付费订单真实性。
        </li> 
        <li>匿名运营统计数据，在软件正常运行时自动采集以下设备通用参数，收集内容包括：软件版本、设备操作系统版本、软硬件配置信息、软件安装时长、功能使用频次、崩溃异常日志、
            运行故障、语言设置、网络信息、出口 IP等数据，全部数据进行去标识化聚合汇总，不识别特定自然人、不构建用户画像、不追踪个体用户行为。
            收集目的：优化软件稳定性、修复程序故障、统计软件活跃数据、防范恶意攻击、管控异常授权滥用、合理调度服务器资源、提升服务质量。
            留存规则：原始运行日志最长保存 90 天，到期自动清除。
        <br>
        </li>
        <li>信息不外泄、不出售，不会采集身份证、通讯录、照片等隐私内容；如需清理备案数据，可发邮件至yyc_xincheng@163.com申请处理。</li>
    </ul>
</p>
...
<p>
    <ul>
        ...
        <li>
            我们在进行激活授权时，为了确保交易有效性，我们或许会进行再次审核或调整授权验证方式，需要您进行配合来进一步确认激活，在审核期间内，您可继续使用原有功能但可能有一定限制。如遇问题，我们将会积极配合您进行解决。
        </li>
        <li>
            用户完成合法激活授权后，即可在合规前提下正常使用本软件（已有版本）。针对激活失败、程序运行异常等问题，我们将尽力提供技术支持予以解决。在现行法律许可范围内，因软硬件环境、系统兼容、用户操作等非我方主观过错引发的问题，我方仅提供问题排查服务，不承担各类直接或间接损失的赔偿责任。
        </li>        
        <li>
            我们保留在未来软件更新中更改访问机制的权利，包括但不限于引入强制登录要求。在此情况下，我们将提前通知用户，并提供合理的过渡期。        
        </li>
        <li>
