# How to configure Nexus access permissions for IDEA plugin Gradle With Me / Gradle With Me Pro?

[Go to Homepage](../README_en.md) / [Go to Pro Homepage](../pro/README_en.md)

This is a scenario where when the Nexus repository has enabled permission access, Gradle With Me(GPVP)/Gradle With Me Pro(GPVP) will not be able to obtain data properly without permission configuration. If your Nexus repository is not configured with permissions, simply keep the plugin's default configuration.
...
**Operation steps:**
  ...
  1. Find the URL to configure user permissions in the user home/gpvp/search-nexus-cache.json
...
The above introduces several ways to configure Nexus access permissions for Gradle With Me(GPVP) / Gradle With Me Pro(GPVP). You can choose according to your actual situation~
